...
class VendorProductVariantDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->addColumn('action', function($query){
            $variantItem = "<a href='".route('vendor.products-variant-item.index', ['productId' => request()->product, 'variantId'=> $query->id])."' class='btn btn-info me-2 text-white'><i class='far fa-edit'></i> Variant Item<a/>";
            $editBtn = "<a href='".route('vendor.products-variant.edit', $query->id)."' class='btn btn-primary'><i class='far fa-edit'></i><a/>";
            $deleteBtn = "<a href='".route('vendor.products-variant.destroy', $query->id)."' id='delete' class='btn btn-danger ms-2'><i class='far fa-trash-alt'></i><a/>";
